...
async def search(query: str, appid: int, cdp_port: int = 18888) -> list:
    ...
    if not ws_url:
        raise RuntimeError("未找到已登录的 Steam 标签页，请在 ModAgent 的 Chrome 里打开并登录 steamcommunity.com")
    q = (query or "").replace("'", "")
    expr = (
        "(async () => {"
...
async def _sub_action(action: str, pubfileid: str, appid: int, cdp_port: int) -> dict:
    ws_url = _steam_ws(cdp_port)
    if not ws_url:
        raise RuntimeError("未找到已登录的 Steam 标签页，请在 Chrome 里打开并登录 steamcommunity.com")
