...
普通版和 Pro 共用完整的 Agent 推荐、核验、计划、安装与回滚闭环；Pro 额外提供订阅级视觉、声音和反馈体验。
...
| 订阅主题、氛围打光、科技控件和反馈核心 | — | ✓ |
| 操作音效、逐项音量校准、回复提醒和 Windows 通知 | — | ✓ |

两版目前都需要用户配置自己的模型与搜索服务 API Key。Pro 构建身份不等于已完成服务端订阅授权；设备激活、在线 entitlement 和商业代码签名仍属于正式售卖前的发行基础设施。

完整边界和不承诺事项见 [普通版与 Pro 功能矩阵](docs/EDITION-COMPARISON.md)。GitHub 普通版不包含 Pro 专有资源和订阅许可文件。
